...
#+BEGIN_SRC julia :noweb-ref wc73-solver
  ...
  function solve_WilsonCowan73(wc73_fn::WilsonCowan73, solver_params)::DESolution
      ...
      new_solver_params = Dict()
      if :dt in keys(solver_params)
	  new_solver_params[:alg] = Euler()
	  new_solver_params[:adaptive] = false
	  new_solver_params[:dt] = solver_params[:dt]
      elseif :stiff in keys(solver_params)
	  if solver_params[:stiff] > 0
	      new_solver_params[:alg_hints] = [:stiff]
	  end
      end

      soln::DESolution = solve(prob; new_solver_params...)
